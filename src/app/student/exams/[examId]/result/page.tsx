...
import type { BloomLevel, ExamResult, TopicAssessment } from '@/entities/exam';

const MOCK_RESULT: ExamResult & { exam_title: string; classroom_name: string } = {
	id: 'r1',
	exam_id: '1',
	session_id: 's1',
...
function BloomBar({ level }: { level: BloomLevel | null }) {
	const index = level ? BLOOM_ORDER.indexOf(level) + 1 : 0;
	const total = BLOOM_ORDER.length;
