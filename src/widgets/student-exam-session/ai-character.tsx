	...
	return (
		<div className="relative flex flex-col items-center justify-center">
			{isSpeaking && (
				<>
					<div
						className="absolute h-72 w-72 animate-ping rounded-full bg-violet-500/10"
						style={{ animationDuration: '1.5s' }}
					/>
					<div
						className="absolute h-60 w-60 animate-ping rounded-full bg-violet-500/20"
			...
			>
				...
				<div
					className={`relative flex h-44 w-44 flex-col items-center justify-center rounded-full shadow-2xl transition-all duration-300 ${
						isSpeaking
				...
				>
					...
					<div className="mb-3 flex gap-5">
						{[0, 1].map((i) => (
							...
							>
								<div
									className={`h-4 w-4 rounded-full bg-slate-900 transition-all duration-200 ${isSpeaking ? 'scale-110' : ''}`}
								/>
					...
					<div className="absolute -top-8 left-1/2 -translate-x-1/2">
						<div className="mx-auto h-8 w-1 rounded-full bg-violet-300" />
						<div
							className={`mx-auto -mt-1 h-4 w-4 rounded-full transition-all duration-300 ${
								isSpeaking ? 'bg-yellow-300 shadow-lg shadow-yellow-300/60' : 'bg-violet-300'
							}`}
						/>
					</div>
				</div>
