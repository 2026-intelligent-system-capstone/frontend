...
import { type ExamQuestionFormValues, bloomLevelOptions, difficultyOptions } from '@/lib/classrooms/exam-presentation';

interface ExamQuestionFormFieldsProps {
...
export function ExamQuestionFormFields({
	questionNumberId,
	questionTextId,
	scopeTextId,
			...
			<div className="grid gap-4 md:grid-cols-3">
				...
				<Select
					...
					onChange={(value) =>
						onChange((prev) => ({
							...prev,
							difficulty: value as ExamDifficulty,
						}))
					}
				>
					<Label>난이도</Label>
			...
			<TextField isRequired className="w-full" name="evaluation_objective">
				...
				<TextArea
					...
					onChange={(event) =>
						onChange((prev) => ({
							...prev,
							evaluationObjective: event.target.value,
						}))
					}
				/>
			</TextField>
				...
				<TextField isRequired className="w-full" name="scoring_criteria">
					...
					<TextArea
						...
						onChange={(event) =>
							onChange((prev) => ({
								...prev,
								scoringCriteria: event.target.value,
							}))
						}
					/>
				</TextField>
